...
pub trait ShareGenerator {
    ...
    fn generate_shares(&self, values: &[Secret]) -> Vec<Vec<Share>> {
    ...
    fn generate_shares_for_batch(&self, batch_values: &[Secret]) -> Vec<Share>;
...
pub trait ShareGeneratorConstruction {
    fn new_share_generator(&self) -> Box<ShareGenerator>;
}
...
impl ShareGenerator for PackedShamirWrapper {
    ...
    fn generate_shares_for_batch(&self, batch_input: &[Secret]) -> Vec<Share> {
